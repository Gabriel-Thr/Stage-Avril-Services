...
class XML_converter:
    ...
    erreur : int
        somme des erreurs dans la classe
    ...
    def CSVtoXML(self, input_file, output_file, fournisseur, code_fournisseur, societe, code_societe, date_application, date_expiration, date_import):
        if not input_file.endswith('.csv'):
            self.logger.warning("Expected a CSV file")
        if not output_file.endswith('.xml'):
            self.logger.warning("Expected an XML file")
        ...
        try:
            ...
            self.logger.warning("File CSV not found")
            return
        ...
        if not (len(date_application) == 8) or not (re.search(checkDate, date_application)):
            self.logger.debug(f"Format de date d'application invalide : {date_application}")
            self.erreur_date += 1
...
class FTPManager:
    ...
    def load_key(self, file_path):
        try:
            ...
            self.logger.error(f'Erreur lors de la lecture du fichier de clé: {e}')
            self.erreur += 1
    ...
    def sendOnFTP(self, ftp_server, ftp_user, ftp_password, ftp_path, file):
        try:
            ...
            self.logger.error(f"Erreur pendant l'envoi du fichier : {e}", exc_info=True)
            self.erreur += 1
...
class ihmOperra:
    ...
    def __init__(self, root):
        ...
        self.conf_path = os.path.join(self.current_directory, 'ftp_operra_tarif_achat.conf')
    ...
    def transformer(self):
        ...
        xml_file = f'OPERRA_{self.extract_name(csv_file)}_{self.date_application_entry.get()}.xml'
        ...
        if csv_file and xml_file:
            try:
                ...
                if self.erreur != 0:
                    ...
                    return self.res.config(text='Erreur pendant la création du fichier XML')
                ...
                self.logger.error(f"Erreur lors de la création du fichier XML: {e}")
                self.erreur += 1
...
if __name__ == "__main__":
    def get_log_level():
        ...
        config = ConfigParser()
        config.read(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ftp_operra_tarif_achat.conf'))

        log_level_str = config.get('Level','log_level',fallback='40')
        log_level = int(log_level_str)

        return LOG_LEVELS.get(log_level)
    
    logging.basicConfig(filename='formulaire_tarif_operra.log', format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=get_log_level())
    root = Tk()
    ihm = ihmOperra(root)
    root.mainloop()